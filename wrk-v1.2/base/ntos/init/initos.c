...
/*
	在这个函数中，除了执行体自身被初始化以外，执行体的各个子组件，包括内存管理器，
	也被初始化。而且，每个子组件都被调用两次初始化，分别对应于阶段0初始化和阶段1初始化
*/
/*++
...

{
    ...
    InitializationPhase = 1; // 设置全局变量为1，标志着当前这次系统引导过程
    						 // 进入内核的阶段 1 初始化
    ...
    //
	// 调用HalInitSystem函数，执行HAL的阶段 1 初始化
    if (HalInitSystem(InitializationPhase, LoaderBlock) == FALSE) {
    ...
    //      of the HAL Layer.
    //
	// 初始化系统时间，这一步必须再HAL的阶段1初始化以后才能进行
    ...
    // complete IO init.
    //
	// 初始化配置管理器，建立起基本的注册表对象操作，把引导时获得的数据转变成
	// 注册表格式，这一步必须再I/O子系统初始化之前完成
    ...
    //
	// 现在系统时间已经正常运行，调用ExInitSystemPhase2，再次初始化执行体组件
    ExInitSystemPhase2();
    ...
    // Monitor State.
    //
	// 执行安全引用监视器（SRM）的阶段1初始化，包括创建安全引用监视器的命令服务
	// 线程。该线程创建一个名为"引用监视器命令端口"的LPC端口，以接收来自
	// lsass.exe进程的命令
    if (!SeRmInitPhase1()) {
	...
	// 在smss进程的句柄上等待，超时值设置为 5s
    Status = ZwWaitForSingleObject(
                ProcessInformation->Process,
                FALSE,
                &OldTime
                );
	// 如果等待成功，则意味着在5s内smss进程退出了，于是调用KeBugCheck
	// 系统崩溃
	// 若等待超时，则认为会话管理器已经正常运行，于是阶段1初始化完成
	// 当前线程蜕变成零页面线程
