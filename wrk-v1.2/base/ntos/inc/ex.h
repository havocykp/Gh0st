...
typedef struct _HANDLE_TABLE_ENTRY {
    ...
    union {
        ...
        ULONG ObAttributes; // 最低三位有特别含义，参见
        					...
        					// 第1位 OBJ_INHERIT 是否将该句柄项拷贝到它们的句柄表中
    ...
    union {
        union {
            ACCESS_MASK GrantedAccess;  // 当Object指向的是一个有效的内核对象类型时,这个字段表示
										// 对内核对象的访问掩码
										
            struct {					// 当NtGlobalFlag中包含
            							// FLG_KERNEL_STACK_TRACE_DB标记时使用
...
typedef struct _HANDLE_TABLE {
    ...
    ULONG_PTR TableCode; // 指向句柄表的存储结构体,这个字段的低2位代表当前句柄表的层数,为0:一层,为1:两层,为2:三层.
    ...
    HANDLE UniqueProcessId;   // 创建句柄表的进程的id，用于回调函数
...
#define HANDLE_TABLE_LOCKS 4
    ...
    //  lock.
    LIST_ENTRY HandleTableList; // 在句柄表链表中的句柄表节点(句柄表链表包含所有进程的句柄表.),表头为HandleTableListHead
    							// 所以的句柄表形成一个链表

    //
    // Define a field to block on if a handle is found locked.
    //
    EX_PUSH_LOCK HandleContentionEvent; // 句柄表互斥锁.
    									// 若在访问句柄时发生竞争，则在此推锁上等待
