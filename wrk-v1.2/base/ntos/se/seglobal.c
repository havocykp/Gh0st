...
LUID SeCreateTokenPrivilege = {0};			// 创建令牌对象
LUID SeAssignPrimaryTokenPrivilege = {0};	// 替换一个进程的令牌
LUID SeLockMemoryPrivilege = {0};			// 锁住内存页面
LUID SeIncreaseQuotaPrivilege = {0};		// 增加一个进程的内存配额
LUID SeUnsolicitedInputPrivilege = {0};		// 已过时，不再使用
LUID SeTcbPrivilege = {0};					// 操作系统使用
LUID SeSecurityPrivilege = {0};				// 管理审计和安全日志
LUID SeTakeOwnershipPrivilege = {0};		// 接管文件和其它对象的所有权
LUID SeLoadDriverPrivilege = {0};			// 加载和卸载设备驱动程序
LUID SeCreatePagefilePrivilege = {0};		// 创建页面文件
LUID SeIncreaseBasePriorityPrivilege = {0};	// 增加调度优先级
LUID SeSystemProfilePrivilege = {0};		// 系统性能分析
LUID SeSystemtimePrivilege = {0};			// 改变系统时间
LUID SeProfileSingleProcessPrivilege = {0};	// 对单个进程进行性能分析
LUID SeCreatePermanentPrivilege = {0};		// 创建永久对象
LUID SeBackupPrivilege = {0};				// 备份文件和目录
LUID SeRestorePrivilege = {0};				// 恢复文件和目录
LUID SeShutdownPrivilege = {0};				// 系统停机
LUID SeDebugPrivilege = {0};				// 调试程序
LUID SeAuditPrivilege = {0};				// 安全审计
LUID SeSystemEnvironmentPrivilege = {0};	// 修改系统的固件环境变量
LUID SeChangeNotifyPrivilege = {0};			// （名字空间中）变化通知，也用于绕过穿越检查
LUID SeRemoteShutdownPrivilege = {0};		// 远程停机
LUID SeUndockPrivilege = {0};				// 从插接站（docking station）移除计算机
LUID SeSyncAgentPrivilege = {0};			// 同步代理（目录服务）
LUID SeEnableDelegationPrivilege = {0};		// 允许计算机帐户或用户帐户被用于委托
LUID SeManageVolumePrivilege = { 0 };		// 管理卷
LUID SeImpersonatePrivilege = { 0 };		// 模仿
LUID SeCreateGlobalPrivilege = { 0 };		// 创建全局对象
