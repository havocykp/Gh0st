...
;    record using specified parameters and finally sets up arguments
;    and calls _KiDispatchException.
;	 这个函数会在栈中分配一个异常记录变量, 使用指定的参数设置异常记录变量,
;	 最后设置调用_KiDispatchException函数的参数, 并调用该函数.
;
;    NOTE:
;
...
align dword
CommonDispatchException proc            ; 函数开始位置
...
;	 这个陷阱处理器用于处理int3单字节指令
;	 A BREAKPOINT exception with additional parameter indicating
;    READ access is raised for this trap if previous mode is user.
;	 断点异常和附加参数指示一个读访问异常触发于 是用户模式
;    陷阱处理器被调用时,如果附加参数是一个读访问异常, 说明这个这个
;    异常在用户模式下产生
; Arguments 参数信息:
;
;    At entry, the saved CS:EIP point to the instruction immediately
;    following the INT 3 instruction.
;	 cs:eip保存的时int3指令的下一条指令的首地址.
;    No error code is provided with the error.
;	 没有错误代码.
; Return value 返回值:
;
