...

{
    ...
    //
	// 如果所属的进程不在内存中（已被换出），则把此线程插入到进程对象的就绪链表中，
	// 并且通知交换线程（KiSwappingThread）执行换入操作
	// 设置线程的状态为就绪状态 
    ...
    if (Process->State != ProcessInMemory) {
        ...
        // stack inswap event.
        //
		// 如果线程的内核栈未在内存中，则增加进程的栈计数，将线程的状态设置为转移状态
		// 并且把线程插入到需要换入内核栈的线程链表中，通知交换线程执行换入操作
        ...
        //
		// 把指定的线程插入到延迟就绪链表中，在函数KiInsertDeferredReadyList中，
		// 线程的状态被设置为延迟的就绪状态
